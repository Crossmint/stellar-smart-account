...
/*
 * This contract is a plugin that will revert on policy install, and on_auth
 * It will also deny transfers greater than 100.
 */
#[contract]
pub struct PluginPolicyContractReverts;
...
#[contractimpl]
impl SmartAccountPlugin for PluginPolicyContractReverts {
    fn on_install(_env: &Env, source: Address) {
        source.require_auth();
        panic!("Plugin installation failed");
    }
...
#[contractimpl]
impl SmartAccountPolicy for PluginPolicyContractReverts {
    fn on_add(_env: &Env, source: Address) {
        source.require_auth();
        panic!("Policy add failed");
    }
...
#[contractimpl]
impl PluginPolicyContractReverts {
...
mod test {
    ...
    fn test_on_auth_increments_counter() {
        let env = setup();
        env.mock_all_auths();
        let contract_id = env.register(PluginPolicyContractReverts, ());
        let client = PluginPolicyContractRevertsClient::new(&env, &contract_id);
