...
fn test_admin_count_underflow_protection() {
    ...
    let admin1_standard = Ed25519TestSigner(admin1.0, SignerRole::Standard(None, 0));
    ...
    let admin2_standard = Ed25519TestSigner(admin2.0, SignerRole::Standard(None, 0));
...
fn test_has_signer_returns_false_when_signer_does_not_exist() {
    let env = setup();
    let admin_signer = Ed25519TestSigner::generate(SignerRole::Admin);
    let non_existent_signer = Ed25519TestSigner::generate(SignerRole::Standard(None, 0));
