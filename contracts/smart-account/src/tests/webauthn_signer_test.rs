...
use crate::auth::proof::{SignatureProofs, SignerProof, WebauthnSignature};
...
use smart_account_interfaces::{Signer, SignerKey, WebauthnSigner};
...
mod webauthn_helpers {
    ...
    pub fn create_webauthn_test_data(env: &Env) -> WebAuthnTestData {
        ...
        let signer = WebauthnSigner::new(key_id, BytesN::from_array(env, &pk_bytes));
        ...
        // Create valid proof
        let valid_proof = SignerProof::Webauthn(WebauthnSignature {
    ...
    pub fn create_invalid_signature_proof(env: &Env, test_data: &WebAuthnTestData) -> SignerProof {
        if let SignerProof::Webauthn(ref valid_sig) = test_data.valid_proof {
...
#[test]
fn test_webauthn_valid_signature_passes() {
    let env = Env::default();
...
#[test]
fn test_webauthn_end_to_end_smart_account_auth() {
    let env = Env::default();

    // Use the same approach as the working webauthn_helpers
    let test_data = webauthn_helpers::create_webauthn_test_data(&env);

    // Register smart account with the webauthn signer from test_data
    let signer = Signer::Webauthn(test_data.signer.clone(), SignerRole::Admin);
    ...
    let signer_key = SignerKey::Webauthn(test_data.signer.key_id.clone());
