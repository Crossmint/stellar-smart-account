...
fn test_signer_with_external_policy() {
    ...
    let test_signer =
        Ed25519TestSigner::generate(SignerRole::Standard(Some(vec![&env, policy]))).into_signer(&env);
    env.register(
        SmartAccount,
        (
...
fn test_update_signer_with_external_polic_lifecycle() {
    ...
    let admin_signer = Ed25519TestSigner::generate(SignerRole::Admin).into_signer(&env);
    let test_signer_1 =
        Ed25519TestSigner::generate(SignerRole::Standard(Some(vec![&env, policy_1]))).into_signer(&env);
