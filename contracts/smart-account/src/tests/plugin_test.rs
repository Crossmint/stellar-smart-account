...
fn test_uninstall_plugin_persists_removal() {
    ...
    let auth_payloads = SignatureProofs(soroban_sdk::map![&env, (admin_key.clone(), admin_proof.clone())]);
    ...
    let count_after_install = env.as_contract(&plugin_id, || {
        DummyPlugin::get_count(&env)
    });
    assert_eq!(count_after_install, 1, "Plugin should have received on_auth call");
    ...
    let count_after_uninstall = env.as_contract(&plugin_id, || {
        DummyPlugin::get_count(&env)
    });
    assert_eq!(count_after_uninstall, 1, "Plugin should NOT receive on_auth after uninstall");
}