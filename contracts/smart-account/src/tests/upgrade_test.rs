...
use soroban_sdk::{
    contract, contractimpl, map, symbol_short, testutils::BytesN as _, vec, Address, Bytes,
...
fn test_upgrade_ed25519_admin_preserved() {
    ...
    assert!(matches!(v1_signer, v1::Signer::Ed25519(_, v1::SignerRole::Admin)));
...
fn test_migrate_secp256r1_to_webauthn() {
    ...
    let (contract_id, _admin_keypair, key_id, pk_bytes) =
        deploy_v1_with_secp256r1_signer(&env);

    // Verify secp256r1 signer exists on v1
    let v1_client = v1::Client::new(&env, &contract_id);
    let v1_signer = v1_client.get_signer(&v1::SignerKey::Secp256r1(Bytes::from_array(
        &env, &key_id,
    )));
    assert!(matches!(
        v1_signer,
    ...
    let new_signer = v2_client.get_signer(&v2::SignerKey::Webauthn(Bytes::from_array(
        &env, &key_id,
    )));
    match new_signer {
        v2::Signer::Webauthn(webauthn_signer, v2::SignerRole::Admin) => {
            assert_eq!(
                webauthn_signer.key_id,
                Bytes::from_array(&env, &key_id)
            );
            assert_eq!(
...
fn test_migrate_without_upgrade_fails() {
    ...
    let signer = v2::Signer::Ed25519(
        v2::Ed25519Signer { public_key: pk },
        v2::SignerRole::Admin,
    );
...
fn test_upgrade_preserves_admin_count() {
    ...
    let (contract_id, admin_keypair, _key_id, _pk_bytes) =
        deploy_v1_with_secp256r1_signer(&env);
    ...
    let webauthn_signer = v2_client.get_signer(&v2::SignerKey::Webauthn(Bytes::from_array(
        &env, &_key_id,
    )));
    assert!(matches!(
        webauthn_signer,
...
fn test_migrate_ed25519_with_time_window_policy() {
    ...
    match signer {
        ...
        other => panic!("Expected Ed25519 Standard signer with empty policies, got {:?}", other),
    }
}
...
fn test_auth_ed25519_works_after_migration() {
    ...
    let signer_key =
        smart_account_interfaces::SignerKey::Ed25519(BytesN::from_array(
            &env,
...
fn test_install_plugin_after_migration() {
    ...
    assert_eq!(count, 1, "Plugin should have received on_auth after migration");
...
fn test_admin_protection_after_migration() {
    ...
    assert!(result.is_err(), "Should not be able to revoke the last admin");
}
