...
use smart_account_interfaces::{
    ExternalPermission, SignerKey, SmartAccountError, SmartAccountPermissionClient,
};

/// Authorization check for `ExternalPermission`. Differs from
/// `ExternalPolicy` in two ways:
///   1. Forwards the evaluating `signer_key` so the external contract can
///      differentiate between signers sharing the same permission contract.
///   2. Uses `try_is_authorized` to contain panics and ABI mismatches —
///      a malicious or buggy permission contract cannot DoS `__check_auth`.
///
...
impl AuthorizationCheck for ExternalPermission {
    fn is_authorized(&self, env: &Env, signer_key: &SignerKey, contexts: &Vec<Context>) -> bool {
        let wallet = env.current_contract_address();
        let client = SmartAccountPermissionClient::new(env, &self.permission_address);
            ...
            Ok(Err(_)) | Err(Err(_)) => {
                env.events().publish(
                    ...
                    PolicyCallbackFailedEvent {
                        policy_address: self.permission_address.clone(),
                    },
                );
                false
            }
...
impl PolicyCallback for ExternalPermission {
